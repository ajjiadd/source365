<?php
// User Login Page - Updated with Auto-Link Fetch
    ...
    if ($user = $result->fetch_assoc()) {
        if (password_verify($password, $user['password_hash'])) {
            if ($user['is_verified'] == 1) {
                ...
                // New: Fetch auto-linked govt docs on login
                $mock_url = "../api/mock_govt_api.php?nid=$nid";  // All types
                $mock_data = @file_get_contents($mock_url);
                if ($mock_data) {
                    $_SESSION['linked_docs'] = json_decode($mock_data, true);  // Store in session for dashboard
                } else {
                    $_SESSION['linked_docs'] = [];  // Empty if no data
                }
...
<script>
    document.getElementById('loginForm').onsubmit = function() { return validateForm('loginForm'); };
</script>
